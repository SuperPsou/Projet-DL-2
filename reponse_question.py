...
datasetTrain = open('dataset_Train.csv','w')
datasetTest = open('dataset_Test.csv','w')


for i in range(taille):


  print("Préparation dataset :",str(round(i/taille*100,1))+"%")
  ...
  for j in range(len(image)):
    for k in range(len(image[0])):
      for l in range(len(image[0])):
        line += str(round(image[j][k][l].item(),3))+" "
  ...
  for j in range(len(question)):
    for k in range(len(question[0])):
      line += str(round(question[j][k].item(),3))+" "
  ...
  ####### TRAIN ET TEST SET
  if i<tailleTrain:
    datasetTrain.write(line +'\n')
    #trainSet.append([image,question,output])
  else:
    datasetTest.write(line +'\n')
